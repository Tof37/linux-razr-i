...
static int mt93013_thread(void *priv)
{
	u16 gain, coarse_itg, fine_itg;
	bool only_dequeue;
	struct mt9e013_device *dev = priv;

	while (1) {
		wait_for_completion(&dev->thread_complete);
		if (unlikely(dev->stopping_thread))
			break;
		mutex_lock(&dev->input_lock);

		if (dev->suspended_exposure_request) {
			dev->suspended_exposure_request = false;
			coarse_itg = dev->suspended_exposure_value.
					integration_time[0];
			fine_itg = dev->suspended_exposure_value.
					integration_time[1];
		...
		mt9e013_set_exposure_filter(dev, coarse_itg,
			fine_itg, gain, only_dequeue);
			 ...
			 const struct i2c_device_id *id)
{
	...
	dev->thread = kthread_create(mt93013_thread, dev, "mt9e013 thread");
	if (!dev->thread) {
		dev_err(&client->dev, "%s: cannot create kthread\n", __func__);
		kfree(dev);
		return -ENOMEM;
	}
	sched_setscheduler(dev->thread, SCHED_FIFO, &param);
